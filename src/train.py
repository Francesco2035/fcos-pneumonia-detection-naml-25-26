...
class Trainer:
    def __init__(
        ...
        self.gradient_every_n_steps = gradient_every_n_steps
    ...
    def _create_data_loaders(self):
        ...
        print(
            f"[LOG] Train samples: {len(train_indices)}"
        )

        print(
            f"[LOG] Validation samples: {len(val_indices)}"
        )

        print("[LOG] Dataloaders created")
    ...
    def _build_targets(
        ...
        """
        Build P3-P7 targets from the dataset ground-truth boxes. predictions is used just to know the feature map shape
        """

        batch_targets = []

        batch_size = predictions[LEVELS[0]]["classification"].shape[0]

        for b in range(batch_size):

            image_targets = {}

            boxes = dataset_targets[b]["boxes"].to(
                self.device
            )
            ...
            for level in LEVELS:
                ...
                _, _, height, width = (
                    predictions[level]["classification"].shape
                )
    ...
    def train_one_epoch(self):
        ...
        # Timing accumulators for the current logging window.
        data_time = 0.0
        forward_time = 0.0
        target_time = 0.0
        backward_time = 0.0

        # Start measuring the time spent waiting for the next batch.
        batch_start = time.perf_counter()

        # Number of batches in the current timing window.
        window_batches = 0
        ...
        for batch_idx, (images, dataset_targets) in enumerate(
            self.train_loader,
            start=1,
        ):
            ...
            for level in LEVELS:
                ...
                # One result per image in the batch.
                level_results = (
                    losses["levels"][level]
                )
            ...
            # Time needed before the next batch is yielded.
            batch_start = time.perf_counter()
            ...
            if (
                ...
                images_per_second = (
                    processed_images / elapsed_epoch
                    if elapsed_epoch > 0
                    else 0.0
                )

                # Average timing per batch in this window.
                if window_batches > 0:

                    avg_data = (
                        data_time / window_batches
                    )

                    avg_forward = (
                        forward_time / window_batches
                    )

                    avg_target = (
                        target_time / window_batches
                    )

                    avg_backward = (
                        backward_time / window_batches
                    )
    ...
    def _log_training_losses(self, stats):
        """
    ...
    def _log_parameter_histograms(self):
        ...
        if (
            self.current_epoch % self.histogram_every_n_epochs
            != 0
        ):
            return
    ...
    def _log_gradient_histograms(self):
        ...
        if (
            self.global_step % self.gradient_every_n_steps
            != 0
        ):
            return
        ...
        for name, parameter in self.model.named_parameters():
            ...
            # Keep the logging focused on the main components.
            if (
                "backbone" in name_lower
                or "fpn" in name_lower
    ...
    def train(self, num_epochs):
        ...
        if self.resume:
            ...
            # If the checkpoint is already at or beyond
            # the requested final epoch, there is nothing to do.
            if self.current_epoch >= num_epochs:

                print(
                    "[LOG] Training already completed "
                    f"up to epoch {self.current_epoch}."
                )
        ...
        for epoch in range(
            ...
            print(
                f"[TRAIN] "
                f"loss={train_stats['total_loss']:.6f}"
            )
            ...
            # The current epoch is completed using the current LR.
            # The new LR is used by the next epoch.
            # -----------------------------------------------------
            ...
            #
            # This is saved AFTER best_metric is updated so that
            # last.pt represents the complete state of the epoch.
            # -----------------------------------------------------

            if (
                epoch % SAVE_EVERY_N_EPOCHS
                == 0
            ):
            ...
            # Flush TensorBoard
            #
            # Makes recent logs visible even if training is stopped.
            # -----------------------------------------------------
